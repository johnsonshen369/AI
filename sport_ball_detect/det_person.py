...
# 使用本地模型路径
#model_path = 'models/yolov5x6.pt'
#model = torch.hub.load('ultralytics/yolov5', 'custom', path=model_path)
model = torch.hub.load('ultralytics/yolov5', 'yolov5s', pretrained=True)

# 定义篮球的标签
BASKETBALL_CLASS = 'person'
...
def detect_and_draw_balls(frame):
    ...
    # 筛选出类别为 'sports ball' 的对象（假设篮球的标签是 'sports ball'）
    ...
    for index, basketball in basketballs.iterrows():
        ...
        print(f"Detected person at: xmin={xmin}, ymin={ymin}, xmax={xmax}, ymax={ymax}, confidence={confidence:.2f}")
